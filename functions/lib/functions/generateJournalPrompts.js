...
exports.generateJournalPrompts = (0, https_1.onCall)({
    ...
    firebase_functions_1.logger.info(`Generating journal prompts (expanded: ${expanded}) for user ${userId}`);
    try {
        // Load user document
...
- Do not include any explanation or meta-commentary, only the prompts themselves.`;
        ...
        if (expanded) {
            ...
            if (prompts.length < 12) {
                ...
                firebase_functions_1.logger.warn(`Got only ${prompts.length} expanded prompts, expected 12-18`);
            }
            ...
            if (prompts.length !== 4) {
                firebase_functions_1.logger.warn(`Got ${prompts.length} initial prompts, expected 4`);
        ...
        if (prompts.length === 0) {
            ...
            if (expanded) {
                prompts.push("What patterns do you notice in your recent entries?", "Describe a moment of joy from this week.", "What question have you been avoiding?", "Write about something you're grateful for.", "What would you tell your past self?", "Describe a challenge and what you've learned.", "What does growth look like for you right now?", "Write about a relationship that has changed.", "What are you curious about exploring?", "Describe a moment of clarity.", "What values are most important to you now?", "How has your perspective shifted recently?");
            }
            else {
                prompts.push("What patterns do you notice in your recent entries?", "Describe a moment of joy from this week.", "What question have you been avoiding?", "Write about something you're grateful for.");
            }
        }
        return {
            prompts,
        ...
        firebase_functions_1.logger.error("Error generating journal prompts:", error);
        if (error instanceof https_1.HttpsError) {
            throw error;
        }
        throw new https_1.HttpsError("internal", `Failed to generate prompts: ${error.message}`);
    }
});
//# sourceMappingURL=generateJournalPrompts.js.map