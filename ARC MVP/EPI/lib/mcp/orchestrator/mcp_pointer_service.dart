...
class McpPointerService {
  ...
  static Future<McpPointer> createPointer({
    ...
    try {
      ...
      final mcpDescriptor = McpDescriptor(
        ...
        duration: descriptor['duration_s'] != null 
            ? Duration(seconds: descriptor['duration_s'])
            : null,
        metadata: descriptor,
      );
      ...
      final mcpIntegrity = McpIntegrity(
        sha256: sha256Hash ?? integrity['sha256'] ?? '',
        crc32: integrity['crc32'] ?? '',
        lastModified: fileExists ? await file.lastModified() : DateTime.now(),
      );
      
      // Create privacy settings
      final mcpPrivacy = McpPrivacy(
        scope: privacy['scope'] ?? 'user-library',
        piiDetected: privacy['piiDetected'] ?? false,
        sharingPolicy: privacy['sharingPolicy'] ?? 'private',
        retentionPolicy: privacy['retentionPolicy'] ?? 'indefinite',
      );
      
      // Create sampling manifest if provided
      McpSamplingManifest? mcpSamplingManifest;
      if (samplingManifest != null) {
        mcpSamplingManifest = McpSamplingManifest(
          keyframes: (samplingManifest['keyframes'] as List<dynamic>?)
              ?.map((kf) => McpKeyframe(
                timestamp: Duration(milliseconds: kf['timestamp'] ?? 0),
                uri: kf['uri'] ?? '',
                metadata: kf['metadata'] ?? {},
              ))
              .toList() ?? [],
          thumbnails: (samplingManifest['thumbnails'] as List<dynamic>?)
              ?.map((thumb) => thumb.toString())
              .toList() ?? [],
          spans: (samplingManifest['spans'] as List<dynamic>?)
              ?.map((span) => McpSpan(
                start: Duration(milliseconds: span['start'] ?? 0),
                end: Duration(milliseconds: span['end'] ?? 0),
                label: span['label'] ?? '',
  ...
  static Future<PointerResolution> resolvePointer(McpPointer pointer) async {
    try {
      final file = File(pointer.sourceUri);
      final exists = await file.exists();
      
      if (!exists) {
        return PointerResolution(
          success: false,
          error: 'Media file not found: ${pointer.sourceUri}',
          content: null,
          metadata: {},
        );
      }
      ...
      if (pointer.integrity.sha256.isNotEmpty) {
        final fileBytes = await file.readAsBytes();
        final hash = sha256.convert(fileBytes);
        final currentHash = hash.toString();
        
        if (currentHash != pointer.integrity.sha256) {
          return PointerResolution(
            success: false,
