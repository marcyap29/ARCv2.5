...
class ChatMultimodalProcessor {
  final OcpImageService _imageService = OcpImageService();
  final OcpVideoService _videoService = OcpVideoService();
  ...
  Future<OcpAnalysisResult> _runOcpAnalysis(String uri, String mime) async {
    if (mime.startsWith('image/')) {
      final result = await _imageService.analyzeImage(uri);
      ...
      final result = await _videoService.analyzeVideo(uri, _getDefaultKeyframePolicy());
      return OcpAnalysisResult(
        mediaType: 'video',
      ...
      final result = await _sttService.transcribeAudio(uri, 'whisper');
      return OcpAnalysisResult(
        mediaType: 'audio',
