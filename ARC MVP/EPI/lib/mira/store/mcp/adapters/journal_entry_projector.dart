...
class McpEntryProjector {
  /// Project all journal entries to MCP format and return as a list of maps
  /// Each map has a 'kind' field ('pointer', 'node', 'edge', or 'embedding')
  /// Used by simple_mcp_service.dart for streaming export
  static Future<List<Map<String, dynamic>>> projectAll({
    ...
    for (final entry in entries) {
      ...
      final pointer = _createPointer(pointerId, entry, text ?? entry.sageAnnotation?.essence ?? '');
      ...
      final edges = _createEdges(entry, nodeData['id'] as String);
  ...
  static Future<Map<String, dynamic>> _createJournalNode(JournalEntry entry, String pointerId) async {
    ...
    final photoMetadata = await extractPhotoMetadataFromContent(entry.content, entry.media);
  ...
  /// Public method for use in tests and external processing
  static Future<List<Map<String, dynamic>>> extractPhotoMetadataFromContent(
    String content,
