  ...
  group('EntryClassifier Tests', () {
    ...
    group('Factual Classification', () {
      test('Classifies simple factual question', () {
        final entry = "I thought Newton's calculus was for prediction, "
      ...
      test('Classifies learning clarification', () {
        final entry = "I learned that derivatives measure rates of change. Is this right?";
        expect(EntryClassifier.classify(entry), EntryType.factual);
      });

      test('Classifies technical question', () {
        final entry = "Does the Kalman filter require matrix inversion?";
        expect(EntryClassifier.classify(entry), EntryType.factual);
      });

      test('Classifies understanding check', () {
        final entry = "Am I understanding this correctly? The integral is the area under the curve?";
        expect(EntryClassifier.classify(entry), EntryType.factual);
      });

      test('Classifies short learning note', () {
        final entry = "I thought X but it's actually Y. Did I get this right?";
        expect(EntryClassifier.classify(entry), EntryType.factual);
      });
    });
    ...
    group('Reflective Classification', () {
      test('Classifies weight tracking entry', () {
        final entry = "I weighed myself yesterday. 204.3 lbs. "
    ...
    group('Analytical Classification', () {
      test('Classifies analytical essay', () {
        final entry = "A theory on AI adoption and its choke point. "
      ...
      test('Classifies theoretical framework', () {
        final entry = "The key distinction between prediction and calculation lies in "
      ...
      test('Classifies third-person analysis', () {
        final entry = "The adoption of new technologies follows predictable patterns. "
    ...
    group('Meta-Analysis Classification', () {
      test('Classifies pattern request', () {
        final entry = "What patterns do you see in my weight loss attempts over the past year?";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });

      test('Classifies temporal comparison', () {
        final entry = "How has my thinking about ARC evolved since I started this project?";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });

      test('Classifies theme inquiry', () {
        final entry = "Looking back at my entries about Eiffel, what themes keep emerging?";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });

      test('Classifies connection request', () {
        final entry = "What connections exist between my sleep quality and work productivity?";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });

      test('Classifies change tracking', () {
        final entry = "Compare my energy levels from last month versus this month.";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });

      test('Classifies explicit ARC request', () {
        final entry = "Based on everything you know about my journey, what insights can you provide?";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });

      test('Classifies pattern analysis request', () {
        final entry = "Analyze my entries and tell me what patterns you notice in my behavior.";
        expect(EntryClassifier.classify(entry), EntryType.metaAnalysis);
      });
    });
    ...
    group('Edge Cases', () {
      ...
      test('Ambiguous short entry defaults to reflective', () {
        final entry = "Today was hard.";
        expect(EntryClassifier.classify(entry), EntryType.reflective);
      });

      test('Emotional analytical essay stays analytical', () {
        final entry = "An analysis of grief in modern society reveals complex patterns. "
      ...
      test('Personal essay with high first-person density stays reflective', () {
        final entry = "I have been thinking about my relationship with failure. "
      ...
      test('Long factual question becomes analytical', () {
        final entry = "I've been studying calculus and I understand that derivatives measure "
    ...
    group('Debug Information', () {
      test('Provides detailed debug info', () {
        final entry = "I'm feeling anxious about the meeting tomorrow. "
      ...
      test('Debug info includes all metrics', () {
        final entry = "What patterns do you see in my progress?";
        final debug = EntryClassifier.getClassificationDebugInfo(entry);
    ...
    group('Classification Consistency', () {
      test('Same entry always gets same classification', () {
        final entry = "I'm trying to understand calculus better. It's challenging.";
      ...
      test('Minor variations produce same classification', () {
        final entry1 = "I'm feeling anxious about tomorrow.";
        final entry2 = "I am feeling anxious about tomorrow.";
        final entry3 = "I'm feeling anxious about tomorrow!";
    ...
    group('Real-world Examples', () {
      test('Weight loss entry', () {
        final entry = "204.3 lbs this morning. Heaviest I've been. "
                     "Need to get back on track with my eating.";
        expect(EntryClassifier.classify(entry), EntryType.reflective);
      });

      test('Technical learning question', () {
        final entry = "I learned that Kalman filters use matrix operations. "
                     "Is matrix inversion required for every update?";
        expect(EntryClassifier.classify(entry), EntryType.factual);
      });

      test('Project analysis', () {
        final entry = "The key insight about ARC's positioning is that it sits between "
      ...
      test('Quick update', () {
        final entry = "Finished the gym. Good workout today.";
        expect(EntryClassifier.classify(entry), EntryType.conversational);
      });

      test('Pattern analysis request', () {
        final entry = "Looking back at my entries about work stress, "
  ...
  group('EntryClassifier Helper Methods', () {
    group('Emotional Density', () {
      test('Calculates high emotional density', () {
        final text = "I feel anxious and scared about this";
      ...
      test('Calculates low emotional density', () {
        final text = "The meeting is scheduled for tomorrow";
        final density = EntryClassifier._calculateEmotionalDensity(text);
    ...
    group('First Person Density', () {
      test('Calculates high first-person density', () {
        final text = "I think my approach is working for me";
      ...
      test('Calculates low first-person density', () {
        final text = "The system works well in most cases";
        final density = EntryClassifier._calculateFirstPersonDensity(text);
        expect(density, equals(0.0));
      });
    });
    ...
    group('Pattern Detection', () {
      test('Detects meta-analysis patterns', () {
        final text = "what patterns do you see in my behavior";
