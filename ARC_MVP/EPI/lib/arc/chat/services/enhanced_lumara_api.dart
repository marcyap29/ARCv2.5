...
class EnhancedLumaraApi {
  ...
  /// Groq service (Llama 3.3 70B) — fallback. Uses current API key from config so keys
  /// updated in Settings are used on the next request (no stale cache).
  GroqService? get _groq {
    final key = _apiConfig?.getConfig(LLMProvider.groq)?.apiKey;
    if (key == null || key.isEmpty) return null;
    // Reuse cached service only if key unchanged to pick up Settings updates
    if (_groqService != null && _groqService!.apiKey == key) return _groqService;
