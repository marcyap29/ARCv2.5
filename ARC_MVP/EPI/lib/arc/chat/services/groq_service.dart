...
class GroqService {
  ...
  Stream<String> generateContentStream({
    ...
    const sendTimeout = Duration(seconds: 90);
    final client = http.Client();
    final response = await client.send(request).timeout(sendTimeout, onTimeout: () {
      client.close();
      throw TimeoutException(
        'Groq API stream request timed out after ${sendTimeout.inSeconds}s.',
        sendTimeout,
      );
    });
  ...
  Future<String> _generate({
    ...
    const timeoutDuration = Duration(seconds: 90);
    final response = await http
      ...
      throw TimeoutException(
        'Groq API request timed out after ${timeoutDuration.inSeconds}s. Check your connection or try again.',
        timeoutDuration,
      );
    });
