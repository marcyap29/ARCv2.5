...
import 'package:my_app/mira/store/mcp/orchestrator/chat_multimodal_processor.dart';
import 'package:my_app/mira/store/mcp/export/chat_mcp_exporter.dart';
import 'package:my_app/mira/store/mcp/models/mcp_schemas.dart';
import 'package:my_app/core/llm/model_adapter.dart';
import 'package:my_app/core/llm/adapters/rule_based_adapter.dart';
import 'package:my_app/echo/providers/llm/llama_adapter.dart';
import 'package:my_app/echo/providers/llm/ollama_adapter.dart';
import 'package:my_app/echo/safety/chat_rivet_lite.dart';
import 'package:my_app/echo/config/echo_config.dart';
import 'package:my_app/echo/rhythms/veil_aurora_scheduler.dart';

/// Multimodal chat service integrating all components
///
/// This is the central orchestrator for LUMARA chat functionality, coordinating:
/// - **Multimodal Processing**: Media analysis through OCP + PRISM pipeline
/// - **LLM Integration**: Multiple provider support (Rule-based, Llama, Ollama, OpenAI, Mistral)
/// - **Safety & Privacy**: RIVET Lite gating and privacy protection
/// - **Scheduling**: VEIL/AURORA nightly maintenance tasks
///
/// ## Architecture Context
/// Part of ARC's chat module (formerly LUMARA). Coordinates between:
/// - POLYMETA (memory storage via MCP)
/// - ECHO (LLM providers, safety, configuration)
/// - PRISM (media analysis)
///
/// ## Data Flow
/// ```
/// User Message
///   → MultimodalChatService.sendMessage()
///   → Media Processing (if media attached)
///   → LLM Provider (via ModelAdapter)
///   → RIVET Lite Safety Check
///   → Response Generation
///   → Memory Storage (via POLYMETA)
/// ```
///
/// ## Initialization
/// Must be initialized at app startup before use. Initialization:
/// 1. Loads ECHO configuration
/// 2. Initializes available LLM adapters
/// 3. Starts VEIL/AURORA scheduler for nightly tasks
class MultimodalChatService {
  ...
  // Core components
  /// Processes media attachments through OCP + PRISM pipeline
  final ChatMultimodalProcessor _processor = ChatMultimodalProcessor();
  
  /// Lightweight RIVET gating for chat safety checks
  final ChatRivetLite _rivetLite = ChatRivetLite();
  
  /// ECHO configuration manager (LLM provider selection, options)
  final EchoConfig _config = EchoConfig.instance;
  
  // Model adapters
  /// Currently active LLM adapter (selected based on config)
  ModelAdapter? _currentAdapter;
  
  /// Map of all available adapters by provider name
  /// Key: provider name (e.g., 'llama', 'ollama', 'openai')
  /// Value: Initialized adapter instance
  ...
  ///
  /// Must be called before using the service. Performs:
  /// 1. ECHO configuration loading (LLM provider settings)
  /// 2. LLM adapter initialization (attempts to initialize all available adapters)
  /// 3. VEIL/AURORA scheduler startup (nightly maintenance tasks)
  ///
  /// Throws if initialization fails - caller should handle gracefully.
  Future<void> initialize() async {
    if (_isInitialized) return;
    
    try {
      // Initialize ECHO configuration
      // Loads LLM provider settings from persistent storage
      await _config.initialize();
      
      // Initialize model adapters
      // Attempts to initialize all available LLM providers
      // Failed adapters are logged but don't block initialization
      await _initializeAdapters();
      
      // Start VEIL/AURORA scheduler
      // Begins nightly maintenance tasks (archive rotation, cache cleanup, etc.)
  ...
  /// Initialize model adapters
  ///
  /// Attempts to initialize all available LLM adapters:
  /// - Rule-based: Always available (fallback)
  /// - Llama: On-device model (requires model file)
  /// - Ollama: Local server (requires Ollama running)
  /// - OpenAI/Mistral: Cloud APIs (require API keys)
  ///
  /// Failed adapters are logged but don't prevent service initialization.
  /// Only successfully initialized adapters are added to the adapters map.
