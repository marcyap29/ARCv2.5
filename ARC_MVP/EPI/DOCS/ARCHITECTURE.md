...
- ✅ **Chat Phase Classification (v3.3.25)**: `ChatPhaseService` classifies LUMARA chat sessions into ATLAS phases using the same `PhaseInferenceService` pipeline as journal entries. Auto-classifies after every assistant response; reclassifies on session revisit. Manual user override with bottom sheet selector. Chat sessions with phase data contribute to `rebuildRegimesFromEntries()` as phase data points alongside journal entries. 3D constellation card in feed.
- ✅ **Crossroads Decision Capture (v3.3.26)**: RIVET-triggered decision detection in chat (`RivetDecisionAnalyzer`). Confirmation prompt → four-step capture flow (decision statement, life context, options, success marker). Decisions stored in CHRONICLE Layer 0 as `entry_type: "decision"`. Monthly synthesis weaves decisions as inflection points. Outcome revisitation via scheduled prompts. New `QueryIntent.decisionArchaeology` for "What decisions have I made?" queries. Export includes `decisions/` directory.
...
- ✅ **SwarmSpace Research Web Search (v3.3.60)**: Research Agent real web search via `swarmspaceRouter` Cloud Function and SwarmSpaceClient; SwarmSpaceWebSearchTool replaces StubWebSearchTool; tier-based plugins (brave-search, tavily-search, semantic-scholar, wikipedia, url-reader); agents_connection_service removed.
...
- ✅ **Cloudflare workflows — clarification + shared research (v3.3.89)**: Repo-root `workers/workflows` adds `clarification_gate.ts` (pre-flight JSON intake) and `research_pipeline.ts` (shared web research with optional `source_documents`, CHRONICLE-aware synthesis); `WorkflowRequest` carries attachment text and `skip_writer_clarification`; EPI Agents run screen handles `clarification_needed` SSE. **`workflow_output_persistence.dart`** exports workflow JSON to `LumaraOutputs/` and syncs to Firestore outputs when authenticated.
- ✅ **Worker research-scope gate + LUMARA research memory trims (v3.3.90)**: `planResearchSubQuestions` in `research_pipeline.ts` runs before web search; `research.ts` / `research_writing.ts` pause with `clarification_needed` (`phase: research_scope`) until `skip_research_scope_clarification`; clarification payloads use `data.phase` (`intake`, `research_scope`, `writing`). EPI: `run_screen` / `worker_service` wiring; `chat_attachment_document_context.dart` + `lumara_assistant_cubit` → Research path; `research_screen` PDF/Word attach; `ResearchAgent` depth-based sub-query caps + 12k document clamp; `search_orchestrator` / `synthesis_engine` / `compactSearchResultsForMemory` reduce retained search payload size; `SwarmSpaceWebSearchTool.contextLookup` for safe PRISM context after async gaps.
- ✅ **Shell tab state + Agents/Writing routing + auth plan doc (v3.3.91)**: `HomeCubit` provided in `app.dart` above `MaterialApp`; `home_view.dart` consumes shared cubit (fixes bottom bar tab sync on pushed routes). Agents: pure-writing intent, conditional output-format section, Writing agent → `WritingScreen`, research→writing handoffs from `run_screen`, CHRONICLE bundle deferred until `skipResearchScopeClarification` when research steps exist, `platforms: null` to worker. Outputs: copy-as-plain export. `authGuard.ts`: Firestore `developers/{uid}` for plan/enforcement document.
- ✅ **Subscription management resilience (v3.3.92)**: `SubscriptionService.getSubscriptionDetails` falls back to `getUserSubscription` for premium signaling when the richer callable is absent; `SubscriptionManagementView` uses a stable premium-access future and clearer billing copy for Stripe vs founder/Firestore premium. RevenueCat iOS uses `kRevenueCatIosApiKeyLive` (public App Store key) with optional `--dart-define=REVENUECAT_IOS_API_KEY` override.
...
- ✅ **Media pick + on-device Vision (v3.3.88)**: `RobustGalleryPicker` wraps `image_picker` with iOS-safe options and FilePicker fallback; `MediaPickAndAnalyzeService` owns `IOSVisionOrchestrator` + `MediaAltTextGenerator` and feeds journal, LUMARA chat-style flows, research document image pick, and `JournalCaptureView` gallery/camera.

### Current Version

- **Application Version**: 1.0.2+3 (from pubspec.yaml)
...
### Alignment with Narrative Intelligence White Paper

The *Narrative Intelligence* white paper (e.g. Narrative_Intelligence.pdf) describes a framework with five integrated modules. **To match the EPI repo, the paper’s architecture should be stated as follows:**

| Paper (conceptual) | EPI implementation |
|--------------------|---------------------|
| **Five modules** | **LUMARA** (interface), **PRISM**, **CHRONICLE**, **AURORA**, **ECHO** |
| LUMARA — Integrative Intelligence Layer | LUMARA Orchestrator coordinates ATLAS, CHRONICLE, AURORA; interface in `lib/arc/` |
| CHRONICLE — Longitudinal Memory and Vector Layer | `lib/chronicle/` + storage in `lib/mira/`; layers 0–N, synthesis, on-device embeddings |
| ATLAS — Developmental Phase Engine (RIVET, SENTINEL) | **ATLAS is implemented within PRISM** (`lib/prism/atlas/`: phase, RIVET, SENTINEL) |
...
*(Rollback and metrics for consolidation work packages are in `DOCS/CODE_SIMPLIFIER_CONSOLIDATION_PLAN.md` § 4–5 and `DOCS/CODE_SIMPLIFIER_METRICS.md`.)*

**2026-02-25 run:** P1-IMPORTS (23 files), QuickActionsService consolidation (widget_quick_actions_service, widget_quick_actions_integration), scan report `CODE_SIMPLIFIER_SCAN_REPORT.md`.
