...
- ✅ **Subscription management resilience (v3.3.92)**: `SubscriptionService.getSubscriptionDetails` falls back to `getUserSubscription` for premium signaling when the richer callable is absent; `SubscriptionManagementView` uses a stable premium-access future and clearer billing copy for Stripe vs founder/Firestore premium. RevenueCat public App Store keys belong in build-time defines, not committed source defaults.
