...
**Action:** Ran Documentation, Configuration Management and Git Backup workflow (claude.md). Trigger: user request (full role + prompt tracking + bug tracker refresh). Commit and push `main` (and merge `main` → `test` when applicable).

**PROMPT REFERENCES AUDIT:** Scanned delta (auth, onboarding, RevenueCat, bootstrap, iOS project, outputs export). No new LLM prompt definitions; `PROMPT_REFERENCES.md` stays **v2.10.1** with last-synced date updated.

**Session / tasking prompts:** Logged in `PROMPT_TRACKER.md` → "Recent tasking prompts (maintenance sessions)".

**Git Backup — Identify what changed (since v3.3.86):** `firebase_auth_service` / `sign_in_screen` — Google via `signInWithProvider` + account picker; `bootstrap` — RevenueCat on iOS after Firebase attempt; `revenuecat_service` — diagnostics; onboarding cubit/sequence + `user_phase_service` + `profile_fields_screen`; `completed_form_export_service`; iOS Podfile static frameworks; `reset_onboarding.dart` dev entry; `.flutter-plugins-dependencies`.

**Bug tracker:** Recent code changes row for v3.3.87; triage note (no new records — mitigations for Google Sign-In 7.x and onboarding stack). Version **3.5.3**.

**Updates:** `CHANGELOG.md` v3.3.87; `CONFIGURATION_MANAGEMENT.md` this entry + inventory; `PROMPT_REFERENCES.md` header; `PROMPT_TRACKER.md` v1.36.0; `bug_tracker.md`; `FEATURES.md` / `ARCHITECTURE.md` / `README.md` / `claude.md` footer.

**Status:** ✅ Doc updates applied; application changes committed with doc sync.

---

### 2026-03-27 - Documentation & Git Backup run (v3.3.86; doc sync 2026-03-27)
...
**PROMPT REFERENCES AUDIT:** Phase 5b writing-screen user prompt expanded in `writing_prompts.dart`; catalog updated with §21 subsection and **v2.10.1**; `PROMPT_TRACKER.md` v1.35.0.

**Git Backup — Identify what changed (since v3.3.85):** Writing screen refactor; `buildPhase5bWritingPrompt` / `phase5bFormatInstructions`; `chronicle_bundle_builder.dart` for real CHRONICLE context in agent runs; agents data/UI/run/worker; `lumara_writing_format_card.dart`; `LumaraUnifiedBottomBar`; outputs detail; report/DOCX export services; profile fields; RevenueCat service; pubspec/iOS project.

**Updates:** `CHANGELOG.md` v3.3.86; `CONFIGURATION_MANAGEMENT.md` this entry + inventory; `PROMPT_REFERENCES.md` v2.10.1; `PROMPT_TRACKER.md` v1.35.0; `bug_tracker.md` Recent code changes + Last Updated; `FEATURES.md` / `ARCHITECTURE.md` / `README.md` / `claude.md` footer.
...
**PROMPT REFERENCES AUDIT:** New §26 — `lumaraModeBindingPreamble` and attachment guidance in `lumara_mode_definition.dart`; injection in `lumara_assistant_cubit.dart`, `enhanced_lumara_api.dart`. PROMPT_REFERENCES.md → v2.9.2.

**Git Backup — Identify what changed (since v3.3.80):** Privacy types extracted to `privacy_settings_types.dart`; `PrivacySettingsService` drives `PiiScrubber` via `applyEgressPrivacySettings`; masking service allowed-types; `prism_adapter` / voice journal guard use `containsBlockingPiiForEgress`; bootstrap privacy init; journal reflection with file attachment extracted text + UI panels; LUMARA mode binding preamble; tests extended.

**Updates:** CHANGELOG.md v3.3.81; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.30.0; bug_tracker.md Recent code changes + Last Updated; FEATURES.md / ARCHITECTURE.md / README.md version alignment.

**Status:** ✅ Doc updates applied with code; commit, push `main`, merge into `test`.
...
**Action:** Ran Documentation, Configuration Management and Git Backup workflow (claude.md §284–501). Trigger: user request. Then commit, push, merge.

**PROMPT REFERENCES AUDIT:** No new LLM prompt definition files. PROMPT_REFERENCES.md unchanged (v2.9.0).

**Git Backup — Identify what changed (since v3.3.78):** research_screen.dart: Research depth selector (Brief/Summary/Deep Dive); research_agent.dart: quick_scan caps sub-queries to 3; synthesis_engine.dart: _depthLabelToSynthesisDepth, synthesis uses researchDepthLabel.

**Updates:** CHANGELOG.md v3.3.79; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.28.0; bug_tracker.md Recent code changes row + Last Updated.

**Status:** ✅ Doc updates applied; commit (docs + code) and push, then merge.
...
**Git Backup — Identify what changed (since v3.3.76):** ARCX export/import: Chronicle pattern index (Chronicle/pattern_index/chronicle_index.json; ChronicleIndexStorage). Chronicle export/import: pattern index in standalone export/import. Unified feed: 150ms delay then _feedRepo.refresh() on import complete. Research: research_artifact_repository upsert/dedupe; feed_repository dedupe. LUMARA: lumara_assistant_cubit switchToSession return bool; llm_provider_factory Ollama provider.

**Updates:** CHANGELOG.md v3.3.77; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.26.0; bug_tracker.md Recent code changes row + Last Updated.
...
**Git Backup — Identify what changed (since v3.3.73):** Uncommitted: lumara_settings_screen, research_screen; output_detail_screen, scan_action_sheet; completed_form_export_service (new); form_matcher, content_brief, research_agent, research_pipeline, synthesis_engine, vision_ocr_screen, parsed_document; profile_fields_screen; arc_onboarding_cubit, arc_onboarding_sequence, phase_reveal_screen; settings_view; prism_service.

**Updates:** CHANGELOG.md v3.3.74; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.23.0; bug_tracker.md Recent code changes row + Last Updated.

**Status:** ✅ Doc updates applied; commit (docs + code) and push, then merge main into test.

---

### 2026-03-10 - Documentation & Git Backup run (v3.3.73; doc sync 2026-03-10)

**Action:** Ran Documentation, Configuration Management and Git Backup workflow (claude.md §284–501). Trigger: user request. Then commit, push, merge main → test.

**PROMPT REFERENCES AUDIT:** No new LLM prompt definition files. PROMPT_REFERENCES.md unchanged (v2.9.0).

**Git Backup — Identify what changed (since v3.3.72):** Uncommitted: lumara_reflection_settings_service, lumara_settings_screen, lumara_splash_screen; arc_onboarding_cubit, arc_onboarding_sequence, arc_onboarding_state, personality_setup_screen, personality_reminder_screen (new); research_screen, writing_screen; outputs (output_detail_screen, outputs_models, outputs_tab_screen, scan_action_sheet, output_folder_tile, output_item_card); agent_tips, research_pipeline, agents_screen, vision_ocr_screen, writing_agent_tab, draft_editor_screen, pipeline_draft; prism_service, swarmspace_client; home_view; swarmspaceRouter.ts. New: lib/lumara/agents/forms/, lib/lumara/profile/; test/lumara/forms/, test/lumara/profile/.

**Updates:** CHANGELOG.md v3.3.73; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.22.0; bug_tracker.md Recent code changes row + Last Updated.
...
**Action:** Ran Documentation, Configuration Management and Git Backup workflow (claude.md §284–501) a third time. Trigger: user request. Then commit, push, merge main → test.

**PROMPT REFERENCES AUDIT:** No new LLM prompt definition files. PROMPT_REFERENCES.md unchanged (v2.9.0).

**Git Backup — Identify what changed (since v3.3.68 commit 311786bd8):** Uncommitted: lumara_assistant_cubit.dart, enhanced_lumara_api.dart, lumara_settings_screen.dart, gemini_send.dart, subscription_service.dart, lumara_subscription_status.dart.

**Updates:** CHANGELOG.md v3.3.69; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.19.0; bug_tracker.md Recent code changes row + Last Updated.

**Status:** ✅ Doc updates applied; commit (docs + code) and push, then merge main into test.

---

### 2026-03-10 - Documentation & Git Backup run (v3.3.68; second run; doc sync 2026-03-10)

**Action:** Ran Documentation, Configuration Management and Git Backup workflow (claude.md §284–501) a second time. Trigger: user request. Then commit, push, merge main → test.

**PROMPT REFERENCES AUDIT:** No new LLM prompt definition files. Persona selector removal is UI/settings only. PROMPT_REFERENCES.md unchanged (v2.9.0).

**Git Backup — Identify what changed (since v3.3.67 commit 5d37b6462):** Uncommitted: arc_subsystem.dart, enhanced_lumara_api.dart, lumara_control_state_builder.dart, lumara_reflection_settings_service.dart, lumara_settings_screen.dart; persona_selector_widget.dart and persona_selector.dart deleted; mcp_export_service.dart, mcp_import_service.dart; advanced_settings_view.dart, settings_view.dart; journal_screen.dart, inline_reflection_block.dart.

**Updates:** CHANGELOG.md v3.3.68; CONFIGURATION_MANAGEMENT.md this entry + inventory; PROMPT_TRACKER.md v1.18.0; bug_tracker.md Recent code changes row + Last Updated.
