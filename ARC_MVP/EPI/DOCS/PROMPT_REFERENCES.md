...
- **Last synced with codebase:** 2026-03-30. Document version: 2.10.3 (§27 unchanged; v3.3.92 audit: subscription UI / doc SOP pack / `claude.md` trim — no new LLM prompt strings).
...
### ECHO On-Device LLM System Prompt (Qwen Adapter)

**Location:** `lib/echo/providers/llm/prompt_templates.dart` (`PromptTemplates.systemPrompt`)

**Used by:** `lib/echo/providers/llm/qwen_adapter.dart` — the ECHO module's on-device Qwen/Gemma LLM adapter. Distinct from `lib/core/prompts_arc.dart` (arc on-device prompt) and `lib/echo/prompts/echo_system_prompt.dart` (ECHO pipeline prompt). Exported via `lib/echo/echo_module.dart`.
...
Rule 1–6: Never guess; name the gap; offer safe paths; mark provenance; no invented details; ask before reaching out.

DATA HANDLING RULES:
- Use ONLY facts and snippets provided in <context>
...
1. One-liner  2. 30-second overview  3. Deeper dive (only if requested)
4. How it helps you today  5. Offer paths (pick one of three)

ENDING FRAME: Close with a single, concrete option the user can accept or decline.
```

**Task-specific templates (`getTaskPrompt(task)`):**

| Task | Focus |
|------|-------|
| `weekly_summary` | Valence trends, key themes, notable moments, trajectory |
| `rising_patterns` | Top terms, delta changes, growth signals, evidence |
| `phase_rationale` | Current phase + ALIGN/TRACE/window scores + entry evidence |
| `compare_period` | Current vs. previous valence, theme shifts, trajectory |
| `prompt_suggestion` | 2–3 phase-appropriate reflection prompts from current patterns |
| `chat` | Helpful, accurate, concise; evidence-based response |
| `default` | General helpful response based on provided context |

**Few-shot examples:** `fewShotExamples` map provides one example each for `weekly_summary`, `rising_patterns`, and `phase_rationale`.

**Context formatter:** `formatContext(facts, snippets, chat)` serializes facts dict, snippets list, and optional chat history into the model input block.
...
## 26. LUMARA Active-Mode Binding Preamble (Personal / Simple / Analysis)

**Source:** `lib/arc/chat/prompts/lumara_mode_definition.dart` — `lumaraModeBindingPreamble` (prepended before `lumaraModeDefinitionBlock` / used with `lumaraModeSwitchBlock` for mode-only switches).

**Injection sites:** Session start in `lib/arc/chat/bloc/lumara_assistant_cubit.dart` (full three-mode block); deep analytical reflection path in `lib/arc/chat/services/enhanced_lumara_api.dart` (same combined string).

**Role:** States that the single `[MODE: …]` line on the user message is authoritative; the three-mode reference is definitions only. Reduces drift into Mode 3 (Analysis) formatting—claims/evidence tables, detached audit tone—when the active tag is `[MODE: Personal]`, including on long attachments. Mode 1 copy in the same file adds guidance to keep Personal voice and provenance for long PDFs/specs unless the user explicitly asks for formal critique.

**Excerpt (binding preamble, abbreviated):**
