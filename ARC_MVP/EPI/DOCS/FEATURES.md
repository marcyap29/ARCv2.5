...
- **Journal file attachments for reflection (v3.3.81)**: Entries with file attachments that have extracted text can run LUMARA reflection without body text; extracted document text is appended to the reflection payload (per-file character cap) and summarized in compact reference panels above the editor. Continue is enabled when attachment reference text exists.
...
- **Agents + Writing shell alignment (v3.3.91)**: **`HomeCubit`** lives at app root so **`LumaraUnifiedBottomBar`** on pushed screens reflects the real shell tab. **Pure writing** requests (draft/post without research cues) suggest a **Writing-only** chain; **Output format** appears only when Writing is predicted; **Writing** agent tile opens **`WritingScreen`**. **Research-only** worker runs offer **Continue in Writing →**; **Writing-only** confirm opens **`WritingScreen`** with format/spec args. Writing format UI: four length tiers (**Short**, **Medium**, **Article**, **Research**), legacy Substack/white-paper ids normalized to article; no per-platform chips on the card. **CHRONICLE** hybrid bundle for worker runs is deferred until after **research_scope** confirmation when research is in the chain. **Outputs** detail: **Copy and paste** plain text (markdown stripped) for research/writing; share sheet path removed from that screen. **ARCX import** from Settings no longer forces navigation to **`/home`** mid-import. **Firebase `authGuard`**: plan/subscription document under **`developers/{uid}`** (not **`users`**).
...
- **Veil Edge Removal; Overview Docs (v3.3.65)**: Veil Edge module and PRISM veil_edge_policy removed; reflection_emotional_analyzer, reflection_pattern_analyzer, aurora_reflection_service removed; journal/voice/timeline/feed refinements; LUMARA_Overview.md, Swarmspace_Overview.md; lumara_api_progress_screen, docx_preview_screen, swarmspace_plugin_approval_store.
...
- **Narrative Intelligence (v3.3.27)**: `DOCS/NARRATIVE_INTELLIGENCE_OVERVIEW.md` — high-level framework (architecture, VEIL cycle, subsystems, vector generation, intellectual honesty, Crossroads). Formal paper LaTeX source: `DOCS/NARRATIVE_INTELLIGENCE_WHITE_PAPER.tex`.
...
- **LUMARA egress + Settings privacy (v3.3.81)**: Stored privacy level/custom settings sync to `PiiScrubber` (`applyEgressPrivacySettings`) on load and save so outbound LUMARA scrubbing matches masking options, enabled PII categories, and detector sensitivity. Bootstrap initializes `PrivacySettingsService` early. PRISM pre-send checks use the same blocking-PII policy as egress scrub.
